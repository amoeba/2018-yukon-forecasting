...
The 2018 value of AMATC was retrieved using the [rnoaa](https://github.com/ropensci/rnoaa) R package which uses the [NOAA CDO](https://www.ncdc.noaa.gov/cdo-web/) API. The daily values of average daily temperature (TAVG) were averaged together to calculate AMATC. This isn't necessarily how NOAA calculates average monthly temperatures but this gets us access to the value earlier and tends to be close enough to the final monthly average temperature to not affect our results.
...
**Intepretation:** An AMATC of -7.3 C is just about in the middle of the historical range for this variable so we should expect an average run timing.
...
I then predicted the 2018 MDJ using the fitted model which came out to be **June `r predicted`**.
